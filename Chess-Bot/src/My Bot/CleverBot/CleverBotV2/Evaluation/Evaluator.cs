...
internal static class Evaluator
{
    ...
    internal static float Evaluate(Board board, Move move, bool whiteMadeMove, float previousScore)
    {
        if (board.IsInCheckmate()) return _checkmate.SideFlip(whiteMadeMove);
        if (board.IsDraw()) return _draw;

        float moveEvaluation = previousScore + EvaluateMove(move, whiteMadeMove);
